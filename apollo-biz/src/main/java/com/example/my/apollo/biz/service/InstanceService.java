...
import com.example.my.apollo.biz.repository.InstanceConfigRepository;
import com.example.my.apollo.biz.repository.InstanceRepository;
      ...
      public Page<Instance> findInstancesByNamespace(String appId, String clusterName, String
        ...
        if (instanceConfigs.hasContent()) {
          Set<Long> instanceIds = instanceConfigs.getContent().stream().map
              (InstanceConfig::getInstanceId).collect(Collectors.toSet());//这个写法 还是.net 的LINQ好用
